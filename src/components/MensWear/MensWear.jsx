...
const MenWear = () => {
  ...
  // Hardcoded dummy products for Men's Wear
  const dummyProducts = [
    {
      id: 1,
      name: "Men's T-Shirt",
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-3xl font-bold mb-4">Men's Wear</h1>
      ...
      <div className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 transform transition-transform duration-500 ${isVisible ? 'translate-x-10' : '-translate-x-full'}`}>
        {filteredProducts.map((product) => (
          <div key={product.id} className="border rounded-lg p-4 cursor-pointer" onClick={() => openModal(product)}>
            <div className="overflow-hidden rounded-lg h-48 mb-2">
              <img 
                src={product.image} 
                alt={product.name} 
